##### Info ####
# Extensible example of calibration procedure.
# Daniel Hoop, 2018-10-16
...
# farmCount = 1, activ=indicating if observation was activated, resp=indicating if observation has responded (i.e. delivered data)
...
df[,c("w_calib","Gew_Calib")] <- 0
df[filtCalib,"w_calib"] <- sampling::calib(Xs=Xs[filtCalib,], d=df[filtCalib,"w0"], q=rep(1,sum(filtCalib)), total=totalPopul, method="linear", description=FALSE, max_iter=500)
df[filtCalib,"Gew_Calib"] <- df[filtCalib,"w_calib"]*df[filtCalib,"w0"]
sum(df[filtCalib,"Gew_Calib"] )
...
m_all <- mean.weight(df[,optVarList[[levelToCheck]]], weights=NULL, index=df[,levelToCheck]); print(m_all)
# Caluclate the weighted mean for the responding farms.
m_resp <- mean.weight(df[,optVarList[[levelToCheck]]], weights=df[,"Gew_Calib"], index=df[,levelToCheck]); print(m_resp)
