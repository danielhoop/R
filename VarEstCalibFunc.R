# Function definition to estimate the variance (imprecision) of estimators based on random samples.
# Swetlana Renner, Daniel Hoop 2018-07-13
# ***************************************
#
# The main function to use is variance.estimate()
# It provides a high level interface to access all functionalities.
# You can use calcXsMultiLevel() as preparation for calibration (with the function sampling::calib()).
#
...
variance.estimate <- function(data, cols=NULL, weights, inclusProbs, index=NULL, indexStrata=NULL, fixedIndex=FALSE, indexOfFixedResult=NULL, indexSep="_", 
                              ...
                              Xs=NULL, Nmin=0, na.rm=TRUE, edit.I.colnames=FALSE) {
  ...
  for(i1 in 1:ncol(levelBin)) { # i1 <- 1
    ...
    if (sum(levelBin[,i1]) < 2) {
      ...
      if (deltaBetweenYears) {
        filt <- levelBin[,i1]
        var1 <- varEstDelta(cbind(data, "pikCol_xc87h23"=inclusProbs, "pikBalCol_xc87h23"=inclusProbsBalanced, "wCol_xc87h23"=weights, "wBalCol_xc87h23"=weightsBalanced, "idCol_xc87h23"=id, "yearCol_xc87h23"=year)[filt,,drop=FALSE],
                            ...
                            year0=year0, year1=year1, Ycols=colnames(data), Xs=if (method=="calib") Xs[filt,,drop=FALSE] else NULL, Nmin=Nmin, na.rm=na.rm)
        if (is.null(dim(var1)))
          var1 <- as.matrix(var1)
        ...
        var1 <- numeric(ncol(data))
        names(var1) <- colnames(data)
        mean1 <- var1
        for(col in colnames(data)) {
          if (startsWith(col,"I(") && endsWith(col,")")) {
            divCols <- gsub(" ", "", strsplit(substr(col,3,nchar(col)-1),"/")[[1]])
            if (length(divCols)!=2)
              stop (paste0("In quotients like 'I(x/y)' only two columns are allowed. Errorneous column: ",paste0(divCols, collapse=", ")))
            divColsNotAvail <- divCols[!divCols%in%colnames(data)]
            if (length(divColsNotAvail)>0)
              stop (paste0("Some columns specified in quotiens like 'I(x/y)' are not available in data: ",paste0(divColsNotAvail, collapse=", ")))
            Yc <- data[,divCols[1]]
            Yd <- data[,divCols[2]]
          ...
          if (length(filt) < 2)
            return (NA_integer_)
          # Calculate variance
          rawVar <- .varEstZaBh(Yc=Yc[filt], Yd=Yd[filt], Xs=if (method=="calib") Xs[filt,,drop=FALSE] else NULL, pik=inclusProbs[filt], w=weights[filt], Nmin=Nmin)
  ...
  if (edit.I.colnames) {
    namesOrig <- .rm.I.from.names(namesOrig)
    cols <- .rm.I.from.names(namesOrig)
  }
...
.varEstZaBh <- function(Yc, Yd=NULL, Xs=NULL, pik, w=NULL, calcMean=FALSE, Nmin=0) {
  ...
  if (length(Yc) != length(w))  stop ("length(Yc) must be euqla ot length(w).")
  if (!is.null(Xs)) {
    if (is.data.frame(Xs))
      Xs=as.matrix(Xs)
    if (is.vector(Xs) & (length(Yc) != length(Xs)))  stop ("length(Yc) must be equal to length(Xs).")
    if (is.matrix(Xs) & (length(Yc) != nrow(Xs))) stop ("length(Yc) must be equal to nrow(Xs).")
  }
  ...
  if (is.null(Yd)) { # Mean or ration of means
    m <- if (calcMean && !is.null(w)) weighted.mean(Yc,w) else NA # Mean can only be calculated if w is given.
    z <- Yc 
    N <- round(sum(1/pik)) # Population size (used in variance formula of mean)
  } else {
    m <- if (calcMean && !is.null(w)) weighted.mean(Yc,w)/weighted.mean(Yd,w) else NA # Mean can only be calculated if w is given.
    Ychat <- sum(Yc/pik)
    Ydhat <- sum(Yd/pik)
    r <- Ychat/Ydhat 
    z <- (Yc - r * Yd)/Ydhat 
...
varEstDelta <- function(dat, Ycols=NULL, YcCol=NULL, YdCol=NULL, Xs=NULL, pikCol, pikBalCol, wCol, wBalCol, idCol, yearCol, year0, year1, Nmin=0, na.rm=FALSE) {
...
#varEstDelta <- function(dat, Ycols=NULL, YcCol=NULL, YdCol=NULL, Xs=NULL, pikCol, pikBalCol, wCol, wBalCol, idCol, yearCol, year0, year1, na.rm=FALSE)
...
.varEstDeltaInnerPreparationForEachCol <- function (dat, datBal, pik, pikBal, w, wBal, Xs, Nmin=0, na.rm=FALSE, I.help.columns=NULL) {
  ...
  for (col in colnames(dat)) {
    notNa <- if (na.rm) which(!is.na(dat[,col])) else 1:nrow(dat)
    notNaBal <- if (na.rm) which(!is.na(datBal[,col])) else 1:nrow(datBal)
    if (!divColInd[col]) {
  ...
  vBal <- lapply(bal, function(x) x$e * wBal - x$A)
  
  return (list(w=w, vary=full, varyBal=bal, vBal=vBal, wBal=wBal, n=nrow(dat)))
}

.varEstDeltaInnerFinalForEachCol <- function (t0, t1) {
  res <- vector("list",length(t0$vary)); names(res) <- names(t0$vary)
  for(i in names(t0$vary)) {
    ...
    corBal<-sum((1-1/t1$wBal)*t1$vBal[[i]]*t0$vBal[[i]])/sqrt(sum((1-1/t0$wBal)*t0$vBal[[i]]^2)*sum((1-1/t1$wBal)*t1$vBal[[i]]^2)) # Korrelationskoeffizent berechnen
