...
.KeyValueStore <- setRefClass(
  # This class stores values received from function calls inside a list.
  # The next time, the value is needed, the class will check the store first, and not execute the function again.
  # The only function of .KeyValueStore that should be called is "setAndGet". All other functions are helper functions.
  # Is used, e.g. inside .onHpdaPc(), .isZaMember(), .agsMachineType()
  Class="valueStore",
...
# Initialize a value store to be accessed by other functions in the namespace.
...
agsPath <- function (path) {
  # This function converts Windows paths to RStudio-Server paths and vice versa.
  ma <- .agsMachineType()
  if (ma[["rstudioserver"]]) {
    ...
    local({
      ...
      if (grepl("^a", info["user"], ignore.case=TRUE)) {
        if (grepl("linux", info["sysname"], ignore.case=TRUE)) {
          res[["rstudioserver"]] <- TRUE
        } else {
          res[["laborpc"]] <- TRUE
        }
      }
      if (grepl("^u", info["user"], ignore.case=TRUE) && grepl("window", info["sysname"], ignore.case=TRUE)) {
        res[["bitpc"]] <- TRUE
      }
      if (sum(unlist(res)) == 0) {
        res[["other"]] <- TRUE
...
.copyFuncs <- function(fromPath, toPath){
  ...
  zaFiles <- c("SqlUtilities.R", "GbUtilities.R")
    for(toPath1 in toPath){
      # SQLUtilities & GBUtilities nur fuer ZA verfuegbar machen!
      if( grepl(agsPath("//evdad.admin.ch/AGROSCOPE_OS/2/5/2/1/3/1"), toPath1) ){
...
.copyZaData <- function(){
  ...
  datFold <- .dataFolder()
  zamain <- agsPath("//art-settan-1000.evdad.admin.ch/ZAMAIN/ZADaten/")
  ...
  if (.agsMachineType()[["rstudioserver"]] && .isZaMember()) {
    # Mount network drives, if they are not yet mounted.
  ...
  if(dir.exists(zamain) && dir.exists(oslwCheck)){
    ...
    if(!dir.exists(datFold)){
      ...
      if (dir.exists(datFold) && .agsMachineType()[["rstudioserver"]]) {
        a <- system(paste0("chmod 700 ", datFold))
        if (a != 0) {
          file.remove(datFold)
          stop ("Fehler! chmod 700 auf ~/data war nicht erfolgreich! *** BITTE HPDA INFORMIEREN ***")
        }
      }
    }
    ...
    files <- matrix(c(
      ...
      paste0(datFold,"SpB_GB_Einzel.RData"),paste0(oslwParent,"4275/GB_Einzel/SpB_GB_Einzel.RData"),
...
(function(){
  ...
  # This variable exists in order to prevent copying of the functions for developing purposes
  ...
  if(.copyHpdaFunctionsToZaNetworkShare && .onHpdaPc() && file.exists(paste0(origPath,"/func.R"))) {#
    ...
    filesize0 <- suppressWarnings(readLines(pathFilesize))
    filesize1 <- paste(file.info(c(paste0(origPath, c("/func.R","/SqlUtilities.R","/GbUtilities.R","/VarEstCalibFunc.R")),
                                   paste0(vokoPath,c("/other_func_Vollk.R"))))$mtime, collapse="  -  " )
...
rm(.mountZaDrivesOnRStudioServer, .createLinkFolderAndLinks, # Don't remove these: .agsMachineType, .isZaMember, .getOS,
...
require.package <- function(...){
  ...
  if( !paste0("package:",pkgName)%in%search() ){
    if( !pkgName%in%rownames(installed.packages()) ){
      install.packages( pkgName , repos='https://stat.ethz.ch/CRAN/' )
    }
...
.install.package <- function (x) {
  ...
  # Package names have to be given as character (quoted).
  for (pkgName in x) {
    if (!paste0("package:",pkgName)%in%search()) {
      if (!pkgName%in%rownames(installed.packages())) {
...
vergleichslohn <- function(region=NULL, jahr=NULL){
  ...
  vgl <- structure(c(42301.656, 38300.148, 35066.52, 43789.056, 39646.848, 
                     ...
                     56609.64, 52255.98, 62055.84, 57003.936, 52619.952, 65854.2, 
                     ...
                     74526.876, 69516.36, 66701.736, 74748.024, 69722.64, 66899.664
  ), .Dim = c(3L, 31L), .Dimnames = list(c("1", "2", "3"), c("1985", 
                                                             "1986", "1987", "1988", "1989", "1990", "1991", "1992", "1993", 
                                                             "1994", "1995", "1996", "1997", "1998", "2001", "2002", "2003", 
...
vergleichszins <- function(jahr=NULL){
  ...
  if(is.null(jahr)) jahr <- colnames(vgl)
  if(is.numeric(jahr) && min(jahr)>1000) jahr <- as.character(jahr)
  return(vgl[,jahr,drop=FALSE])
}

ahv_anteil_betrieb <- function(jahr=NULL){
  BHJ <- 2016
  vgl <- matrix(rep(60,BHJ-2014+1),nrow=1);
...
ahv_satz_BG <- function(jahr=NULL){
  BHJ <- 2016
  vgl <- matrix(rep(12.5,BHJ-2014+1),nrow=1);
  colnames(vgl) <- 2014:BHJ
...
h <- function(x, n=6) {
  ...
  if(length(dim(x))==3) {
    ...
    if(is.data.frame(x)) {
      ...
      cn1 <- colnames(x)
      rn1 <- rownames(x)
      x <- as.data.frame(rbind(paste0("<", substr( sapply(x,function(x)class(x)), 1,3), ">"),
...
dput2 <- function (x) {
  if (is.data.frame(x)) {
    ...
    str <- paste0("if (TRUE) { asdofiue <- data.frame(", str, ", stringsAsFactors=FALSE); ",
...
getCol <- function (obj, col=NULL, expr=NULL) {
  ...
  #' @param expr Optionally instead of 'col', an expression can be given that should be evaluated.
  #' E.g. you could do this: getCol(df, expr=with(df, a <- b))
...
merge.matrices <- function(..., fill="", nbreak=0, aligned=c("left","right"), integrate.dimnames=c("no","rowcol","row","col"), func=c("rbind","cbind")) {
  ...
  # Transform not matrices to matrices
...
transl.ths <- function(x, give.tab=FALSE){
  ...
  ths.name <- c("Agro-Treuhand R?tti AG", "Agro-Treuhand Schwand", "Agro-Treuhand Berner-Oberland",  "Agro-Treuhand Emmental", "Agro-Treuhand Aargau", "Agro-Treuhand Thurgau AG",
...
transl.mm <- function(x, gsub=FALSE, excel.format=FALSE){
  ...
  transmm_list <- .valueStoreInstance$setAndGet(
...
transl.spb.330col.340row <- function(x, reverse=FALSE, give.tab=FALSE, nice.names=FALSE){
  ...
  if(nice.names) rownames(t1) <- c("Milchkuehe","Mutterkuehe","Kaelbermast","Rindviehmast","Haltung fremdes Rindvieh","Rinder- & Kaelbermast fuer Dritte","Uebriges Rindvieh",
...
transl.spb.320row <- function(x, give.tab=FALSE){
  ...
  rownames(t1) <- c("Weizen (Brotgetreide)", "Roggen (Brotgetreide)", "Dinkel (Brotgetreide)", "Emmer, Einkorn", "Mischel Brotgetreide", "Gerste (Futtergetreide)", "Hafer (Futtergetreide)", "Triticale (Futtergetreide)", "Futterweizen",
                    "Mischel Futtergetreide", "Koernermais", "Silo- & Gruenmais", "Saatmais", "Hirse", "Kartoffeln", "Zuckerrueben", "Futterrueben", "Raps zur Speiseoelgewinnung", "Soja", "Sonnenblumen zur Speiseoelgewinnung", "Uebrige Oelsaaten",
                    "Ackerbohnen zu Futterzwecken", "Eiweisserbsen zu Futterzwecken", "Uebrige Koernerleguminosen", "Tabak", "Einjaehrige gaertnerische Freilandkulturen", "Einjaehrige nachwachsende Rohstoffe", "Freiland Frischgemuese",
                    "Freiland Konservengemuese", "Einjaehrige Beeren (z.B Erdbeeren)", "Einjaehrige Gewuerz- & Medizinalpflanzen", "Uebrige Ackerkulturen", "Buntbrache", "Rotationsbrache", "Saum auf Ackerflaeche", "Ackerschonstreifen",
                    "Futterbau (ohne Silomais, Futterrueben und Samenproduktion)", "Samenproduktion Futterbau", "Reben", "Obst, Streuobst ohne Flaeche", "Obstanlagen", "Mehrjaehrige Beeren", "Mehrjaehrige nachwachsende Rohstoffe",
                    "Hopfen", "Mehrjaehrige Gewuerz- & Medizinalpflanzen", "Gemuese-Dauerkulturen", "Christbaeume, Baumschulen und aehnliches", "Uebrige Dauerkulturen", "Gewaechshaus- und Tunnel-Frischgemuese", "Gaertnerische Kulturen in geschuetztem Anbau",
                    "Weitere Flaechen innerhalb der LN", "Wald", "Weitere Flaechen ausserhalb der LN")
...
transl.EB.MML <- function(x, inverse=FALSE, give.tab=FALSE) {
  ...
  uebers_tab <- .valueStoreInstance$setAndGet(
...
transl.accountName <- function (x, lang=c("de", "fr")) {
  ...
  xml <- .valueStoreInstance$setAndGet(
  ...
  if (length(res) != length(x))
    stop ("There are more translations than entries in 'x'. An error happened.")
  return (res)
}
...
#x <- "Differenz = Privatbezüge für Privatkosten"
repl.utf8 <- function(x) {
  # This function recodes UTF8 (e.g. ö to ?) by using a translation matrix & gsub()
...
diffInDataset <- function (dat1, dat2, colsToCheck=NULL, tolerance=1e-9) {
  ...
  for (col in colsToCheck) {
    if (mode(dat1[,col]) != mode(dat2[,col])) {
      ...
      errors[,col] <- round(abs(dat1[,col] - dat2[,col]), digits=invTol) != 0
    } else {
...
assertDatasetEquality <- function (dat1, dat2, idCols=c("id","year"), colsToCheck=NULL, regexToCheck=NULL, colsShowInReport=NULL, showAllDiffCols=FALSE, tolerance=1e-6, outFile=NULL, openOutFile=FALSE, sep=";", header=TRUE, ...) {
  # This function asserts that two data.frames/matrices dat1 and dat2 are equal. If not, a warning is shown and the differences in the data sets is returned.
  # Arguments
  # dat1:         A data.frame/matrix or the path to a text file that can be read with 'read.table' or the path to a RData file that contains a single R object.
  # dat2:         A data.frame/matrix or the path to a text file that can be read with 'read.table' or the path to a RData file that contains a single R object.
  # idCols:       The columns that uniquely identify a observation in each row.
  # colsToCheck:  The columns that should be checked for differences in dat1 and dat2.
  # regexToCheck: A regular expression defining the columns that should be checked for differences in dat1 and dat2.
  # colsToShowInReport: Additional columns that should be shown in the report (apart from the columns named in idCols and the errorneous cols).
  # tolerance:    The tolerance with which equality of numeric values should be checked.
  # outFile:      Optional: The name of the file that should be created which will contain all errorneous observations and columns.
  # sep:          The column delimiter used in read.table and write.table
  # header:       Logical indicating if the files to read in have a header
  # ... :          Additional arguments passed to write.table
  # Value
  # NULL if no differences were detected.
  # A data.frame containing all errorneous rows and columns, if differences were detected. The data.frame contains only the differences in the cells.
  # Details
  # Either colsToCheck or regexToCheck must be given, otherwise an error is thrown.
  ...
  if (!all(idCols%in%colnames(dat1)))
    stop ("Some columns defined in idCols are missing in dat1. Notice that colnames should be defined, not a vector of ids.")
  if (!all(idCols%in%colnames(dat2)))
    stop ("Some columns defined in idCols are missing in dat2. Notice that colnames should be defined, not a vector of ids.")
  ...
  if (length(colsMissing) > 0)
    warning (paste0("Some columns in dat1 are missing in dat2: ", paste0(colsMissing, collapse=", ")))
  ...
  #
  allIds <- table(c(dat1[,uid], dat2[,uid]))
  allIds <- names(allIds)[allIds==2]
  ...
  if (any(!charCol)) {
    diff[,!charCol] <- dat1[,!charCol] - dat2[,!charCol]
  }
  ...
  if (any(charCol)) {
    diff[,charCol] <- mapply(
      function(x,y) {
        res <- rep("", length(x))
        fill <- x != y
        res[fill] <- paste(x[fill], y[fill], sep=" <-1|2-> ")
        return (res)
      },
...
####
# x <- c(0.1, 0.111, 1000); startDigits=5; tol=1/startDigits
#max.digits(c(0.1,0.111,10000))
...
#g1roup.by.fix.scale(c(0.2,1,10,1000), c(0.1,9.3,20,2000), give.names=TRUE, equal.length.names=TRUE)
...
file.copy <- function (from, to, overwrite = recursive, recursive = FALSE, copy.mode = TRUE, copy.date = TRUE) {
  # This is a fast version of file.copy implemented by fs::file_copy
  # However, it behaves the same as does utils::file.copy
  # The default arguments are set to match fs::file_copy behaviour. If any of 'recursive', 'copy.mode' or 'copy.date' are not set to default,
  # then base::file.copy will be called (not fs::file_copy).
  
  # Normal function call, if not default settings for some arguments.
  if (recursive || !copy.mode || !copy.date)
    return (base::file.copy(from=from, to=to, overwrite=overwrite, recursive=recursive, copy.mode=copy.mode, copy.date=copy.date))
  
  # Else, prepare everything for fs::file_copy
  # Install fs package if necessary
  for (pkgName in "fs") {
    if (!paste0("package:",pkgName)%in%search()) {
      if (!pkgName%in%rownames(installed.packages())) {
        install.packages(pkgName)
...
.isNecessaryToUseTmpDir <- function(x, file, sizeThreshold=1400000) {
  ...
  return( is.character(file) && substr( getFullyQualifiedFileName(file) ,1,1)%in%c("/","\\") && object.size(x)>sizeThreshold )
}
...
read.table <- function(file, header=FALSE, sep="", fileEncoding="", ..., choose.columns=NULL, RtoolsBin=agsPath(paste0(c("C:/Program Files","C:/Tools","//evdad.admin.ch/AGROSCOPE_OS/2/5/2/1/3/1/4269/B0000/driversEtc","//evdad.admin.ch/AGROSCOPE_OS/0/4/5/3811/Dokumente/R_Uebung/sonstigeFiles/Rtools/bin"),"/Rtools/bin"))){
  ...
  # RtoolsBin:      if !is.null(choose.columns), then Rtools must be installed because the RtoolsBin/cut.exe and some dll libraries are needed.
  ...
  # choose.columns may be implemented with scan(file, what=list(Name="", Age=0, ...), sep=";", quiet=TRUE). Try it! https://www.datacamp.com/community/tutorials/importing-data-r-part-two
  ...
  if("connection"%in%class(file) || class(file)!="character" || !grepl("^.*(.gz|.bz2|.tar|.zip|.tgz|.gzip|.7z)[[:space:]]*$", file) || !file.exists(file) || !is.null(choose.columns) ) {
    # Simple reading in without choosing columns
    if(class(file)[1]!="file" || !file.exists(file) || is.null(choose.columns)){
      return( utils::read.table(file=file, header=header, sep=sep, fileEncoding=fileEncoding, ...) )
      ...
      if(is.na(RtoolsBin)) stop(paste0("If !is.null(choose.columns), then the R toolset and Cygwin DLLs of Rtools must be available. The Rtools binaries folder was not found at location: ", RtoolsBin))
      ...
      return( utils::read.table(pipe(paste0(RtoolsCut, " -f",paste0(choose.columns,collapse=","), " -d", sep," ", getFullyQualifiedFileName(file)) ), header=header, sep=sep, fileEncoding=fileEncoding, ...))
    }
...
view <- function(x, names=c("col","rowcol","row","no"), nrows=-1, ncols=-1, fastViewOfSubset=TRUE, folder=NULL, quote=FALSE, na="NA", sep=";", decimal.mark=".", openFolder=FALSE, ...){
  ...
  if (.agsMachineType()[["rstudioserver"]]) {
    View(x)
    return (invisible(NULL))
  }
...
load.spe <- function() {
  ...
  pfad2 <- agsPath("//evdad.admin.ch/AGROSCOPE_OS/2/5/2/1/3/3/4276/alldata/SpE.RData")
  pfad <- if(file.exists(pfad1)) pfad1 else pfad2
  message(paste0("Daten werden aus folgendem Verzeichnis geladen: ", pfad))
  spa <- load2(pfad)
  ...
  if(FALSE){
    ...
    if (Sys.time() > strptime("14.8.2018 15:00", "%d.%m.%Y %H:%M")) {
      ...
      warnStopFunc <- warning
    }
    pfad_CRM_plaus_t0 <- agsPath(paste0("//art-settan-1000.evdad.admin.ch/ZAMAIN/ZADaten/SpE/Liste_Plausible/B",BHJ,"/"))
    ...
    plauslist <- read.table(agsPath(paste0("//art-settan-1000.evdad.admin.ch/ZAMAIN/ZADaten/SpE/Liste_Plausible/B",BHJ,"/",fold,"/Plausible_B",BHJ,".csv")),  sep=";", skip=2, header=TRUE, stringsAsFactors=FALSE, quote = "\"")
...
load.spe.pers <- function(){
  pfad <- agsPath("//evdad.admin.ch/AGROSCOPE_OS/2/5/2/1/3/3/4276/Personen/SpE_Personen_Indexiert.RData")
  message(paste0("Daten werden aus folgendem Verzeichnis geladen: ", pfad))
  return(load2(pfad))
}
# SpE GB
load.spe.gb <- function() {
  ...
  pfad2 <- agsPath("//evdad.admin.ch/AGROSCOPE_OS/2/5/2/1/3/3/4276/GB_Einzel/SpE_GB_Einzel.RData")
  pfad <- if(file.exists(pfad1)) pfad1 else pfad2
  message(paste0("Daten werden aus folgendem Verzeichnis geladen: ", pfad))
  return(load2(pfad))
}
...
load.spb <- function() {
  ...
  pfad2 <- agsPath("//evdad.admin.ch/AGROSCOPE_OS/2/5/2/1/3/3/4275/alldata/SpB.RData")
  pfad <- if(file.exists(pfad1)) pfad1 else pfad2
  message(paste0("Daten werden aus folgendem Verzeichnis geladen: ", pfad))
  return(load2(pfad))
}
...
load.spb.gb <- function() {
  pfad1 <- paste0(.dataFolder(),"SpB_GB_Einzel.RData")
  pfad2 <- agsPath("//evdad.admin.ch/AGROSCOPE_OS/2/5/2/1/3/3/4275/GB_Einzel/SpB_GB_Einzel.RData")
  pfad <- if(file.exists(pfad1)) pfad1 else pfad2
  message(paste0("Daten werden aus folgendem Verzeichnis geladen: ", pfad))
  return(load2(pfad))
}
...
load.spb.pers <- function(){
  pfad <- agsPath("//evdad.admin.ch/AGROSCOPE_OS/2/5/2/1/3/3/4275/Personen/SpB_Personen_Indexiert.RData")
  message(paste0("Daten werden aus folgendem Verzeichnis geladen: ", pfad))
  return(load2(pfad))
}
...
load.gb <- function() {
  ...
  pfad2 <- agsPath("//evdad.admin.ch/AGROSCOPE_OS/2/5/2/1/3/3/4273/GB/GB.RData")
  pfad3 <- agsPath("//evdad.admin.ch/AGROSCOPE_OS/2/5/2/2/3583/Resultate/00-00-00_Zusatzdaten/Grundlagenbericht_RefB/GB.RData")
...
load.agis <- function(year=2015){
  ...
  pfad2 <- agsPath(paste0("//art-settan-1000.evdad.admin.ch/ZAMAIN/ZADaten/AGIS/",year,"/AGIS_BFS_",year,".RData"))
  pfad <- if(file.exists(pfad1)) pfad1 else pfad2
  message(paste0("Daten werden aus folgendem Verzeichnis geladen: ", pfad))
  return(load2(pfad))
}
...
load.cost <- function(years=2014, ignore_P_cols=TRUE, non_aggr=FALSE, filter_expression=NULL, parentDir=NULL){
  ...
  # ignore_P_cols = Logical indicating if all columns with the ending "_P" (proportional allocation) should be ignored when reading in the data
  ...
  if(is.null(parentDir)){
    parentDir <- c("C:/Users/U80823148/_/Data/Voko", "C:/Users/A80823148/_/ME/ME_data_out/data_final")
...
getXmlNodeIndexByNameAndAttr <- function (xml, name, attr) {
  # Function to get the index of a xml node by providing the name and attribute of that node.
  # Arguments
  # xml = The xml root node (derived by XML::xmlRoot(XML::xmlParse(filename)))
  # name = The name of the node that shall be searched. length(name) must be 1.
  # attr = The attributes of the node that shall be searched. length(attr) can be greater than 1.
  # Value
  # A numeric vector with the indexes that match the given search criteria.
...
if(FALSE) p1lm.within.between_DELETE <- function(data, index, Y, timeVariantX, timeInvariantX=NULL, timeDummies=NULL, ...){
  ...
  if(FALSE) {
    # Wie koennte man die Freiheit einfuegen, nested random effects zu formulieren und trotzdem das einfache plm-interface behalten?
...
plm.within.between <- function(data, index=NULL, randomEffects=NULL, Y, timeVariantX, timeInvariantX=NULL, timeDummies=NULL, weights=NULL, summaryType=c("Satterthwaite","Kenward-Roger"), ...){
  ...
  require.package(MuMIn) # https://stats.stackexchange.com/questions/46160/r-square-in-mixed-model-with-random-effects
  ...
  # Check if all entries in randomEffects correspond to colnames in data.
  ...
  # If the weights are NULL, then make them inversely proportional to the number of occurences of a observation.
  if(is.null(weights)) {
  ...
  pre.plm.data.transformation <- function(data, timeVariantX, effects){
    ...
    for(i in 1:length(effects)) {
      ...
      # Diese Variablen ueber den Loop hinweg akkumulieren, damit sie nachher in den Funktions-Output integriert werden koennen.
  ...
  # ... Create the summary table
  for(i in summaryModels){ # i <- names(su1)[1]
    # Summary can be NULL if index was not given. In that case, leave table column empty.
    if (!is.null(su1[[i]])) {
      toRows <- match(
        # replace.values() to put all _d rows into the normal rows for simpler overview.
        replace.values(d$dVarName, d$varName, rownames(su1[[i]][["coefficients"]])),
      ...
      if (any(is.na(toRows)))
        stop(paste0("After formatting the I() colnames, they do not fit to the original ones. Please adapt the original ones like this:", paste0(rownames(su1[[i]][["coefficients"]])[is.na(toRows)],collapse=", ")))
      # Add Rsquared an P values into the coefficients table. Only for plm summary.
      toCols <- which(colnames(suRes)==i)
...
#w1ilcox.or.chisq(data.frame(rnorm(1:100),c(rep(1,50),rep(2,50))),c(rep(1,50),rep(2,50)))
