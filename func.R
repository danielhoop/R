...
x=data[,cvars[1]]
col2=colorRampPalette(gradientColors)(100)[findInterval(x,seq(min(x),max(x),length.out=100))]
} else if(!is.null(col)) {
...
par(usr=c(0,1,0,1))
r=round(cor(x,y,method=cor.method),digits=digits)
...
binwidth=h[[grp[1]]]$mids[2]-h[[grp[1]]]$mids[1]
...
data=data[!names(data)%in%delnames]
}
if(edit.I.colnames){
...
y1=(y0-26782.5)/10000
x=200147.07+308807.95*x1+3745.25*y1^2+76.63*x1^2+119.79*x1^3-194.56*y1^2*x1
y=600072.37+211455.93*y1-10938.51*y1*x1-0.36*y1*x1^2-44.54*y1^3
x=round(x)
y=round(y)
res=c(y=y,x=x)
return(res)
...
"
<!--
HerkunftdesBeispiels:
https://developers.google.com/maps/documentation/javascript/examples/icon-complex
Alleswasmit
shape:shape,
zutunhat,einfachausdemCodel?schen.
-->
<!DOCTYPEhtml>
<html>
<head>
...
<meta charset='utf-8'>
<title>KoordinatenUebersicht</title>
<style>
html,body,
height:100%;
margin:0px;
...
//Addmarkerstothemap
//MarkersizesareexpressedasaSizeofX,Y
//wheretheoriginoftheimage(0,0)islocated
//inthetopleftoftheimage.
for (var i = 0; i < locations.length; i++) {
varlocationProperty=locations[i];
varmyLatLng=newgoogle.maps.LatLng(locationProperty[1],locationProperty[2]);
varimage={
...
divpoint=divpoint[-length(divpoint)]
...
write.table(diff,file=outFile,sep=sep,quote=FALSE,col.names=TRUE,row.names=FALSE)
if(openOutFile){
Sys.sleep(0.3)
browseURL(outFile)
}
...
result=result[,c((1+ncol_df2+1)
,(1+ncol_df2+1+1):ncol(result)
,1
,2:(1+ncol_df2))
]
}
...
mf=match.call()
criterium=eval(mf$criterium,envir=data)
...
tryCatch({
utils::write.table(x=1,file=file,fileEncoding=fileEncoding,...)
...
RtoolsBin=RtoolsBin[file.exists(RtoolsBin)][1]
...
if(!remove.middle.rows.cols&last.col.NA){
colNAs1=rev(colNAs)
colNAs2=numeric()
setNA=1
while(setNA!=0){
...
if(!remove.middle.rows.cols&last.row.NA){
rowNAs1=rev(rowNAs)
rowNAs2=numeric()
setNA=1
while(setNA!=0){
...
if(isTRUE(!file.info(zipfile)$isdir))suppressWarnings(file.remove(zipfile))
...
form=model$formula
class(form) <- "formula"
newdata=model.matrix(form,newdata)[,-1,drop=FALSE]
}
...
if(!root){roots=rep(0,length(independent)*degree)}
if(is.null(roots)&root){roots=rep(1,length(independent)*degree)}
...
interc+sum(mapply(prod,polys(x,degree=degree,root=roots[i]),coefs)),
xval[,i],
